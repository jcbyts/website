import { c as create_ssr_component, f as each, e as escape, v as validate_component, g as add_attribute } from "./app-1a513ba3.js";
...
<div class="${"cv"}"><h1>CV</h1>
  ...
  <div class="${"list"}">${each(cv, ({ title, items }, i) => `<div class="${"section"}"><h3>${escape(title)}</h3>
    ...
    (title.toLowerCase() === "publications" ? "-ml-14" : "") + " " + (title.toLowerCase() !== "publications" ? "-ml-8" : "") + " " + (title.toLowerCase() === "publications" ? "space-y-12" : "") + " " + (title.toLowerCase() !== "publications" ? "space-y-6" : "")
          ...
          ${each(items || [], ({ title: title2, when, who, authors, where, tag, footnote }) => `<div class="${"px-8 py-5 flex justify-between"}"${add_attribute("key", title2, 0)}><div class="${""}"><h3 class="${"mt-0"}">${escape(title2)}</h3>
                ${each([who, authors].filter((d) => d), (d) => `<div class="${"text-gray-500"}">${escape(d)}</div>`)}
                ${each([where, tag, footnote].filter((d) => d), (d) => `<div class="${"text-gray-500"}">${escape(d)}</div>`)}</div>
              <div class="${"font-mono text-gray-500 mt-3"}">${escape(when)}</div>
            </div>`)}</div>
      </div>`)}</div>
</div>`;
});
